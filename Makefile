...
ocm/cluster/create:
	@${OCM} post /api/clusters_mgmt/v1/clusters --body=ocm/cluster.json

.PHONY: ocm/cluster/delete
ocm/cluster/delete:
	@${OCM} post /api/clusters_mgmt/v1/clusters --body=/myhome/cluster.json

.PHONY: ocm/cluster.json
ocm/cluster.json: export OCM_CLUSTER_NAME := $(USER)
ocm/cluster.json: export OCM_CLUSTER_REGION := "eu-west-1"
ocm/cluster.json: export OCM_CLUSTER_VERSION := ""
ocm/cluster.json:
	@mkdir -p ocm
	sed "s/OCM_CLUSTER_NAME/$(OCM_CLUSTER_NAME)/g" ocm-cluster-template.json | \
	sed "s/OCM_CLUSTER_REGION/$(OCM_CLUSTER_REGION)/g" | \
	sed "s/OCM_CLUSTER_VERSION/$(OCM_CLUSTER_VERSION)/g" > ocm/cluster.json
